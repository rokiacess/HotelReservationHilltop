<?php
	...
	if(ISSET($_POST['add_form3'])){
		...
		if($row > 0){
			...
			$conn->query("UPDATE `transaction` NATURAL JOIN `guest` SET `firstname` = '$firstname' , `lastname` = '$lastname', `room_no` = '$room_no', `days` = '$days',  `status` = 'Check In', `checkin_time` = '$time', `checkout` = '$checkout', `bill` = '$total3' WHERE `transaction_id` = '$_REQUEST[transaction_id]'") or die(mysqli_error());
