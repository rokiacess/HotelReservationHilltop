<?php
	...
	if(ISSET($_POST['add_form'])){
		...
		if($row > 0){
			...
			$total = $fetch2['price'] * $day;
			$total2 = 800 * $extra_bed;
			$total3 = $total + $total2;
			$checkout = date("Y-m-d", strtotime($fetch['checkin']."+".$day."day"));
			$conn->query("UPDATE `transaction` SET `room_no` = '$room_no', `day` = '$day', `extra_bed` = '$extra_bed', `status` = 'Check In', `checkin_time` = '$time', `checkout` = '$checkout', `bill` = '$total3' WHERE `transaction_id` = '$_REQUEST[transaction_id]'") or die(mysqli_error());
