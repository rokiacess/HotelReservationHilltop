<?php
	...
	if(ISSET($_POST['add_guest'])){
		...
		$checkin = $_POST['date'];
		$conn->query("INSERT INTO `guest` (firstname, middlename, lastname, address, email, contactno) VALUES('$firstname', '$middlename', '$lastname', '$address', '$email','$contactno')") or die(mysqli_error());
		$query = $conn->query("SELECT * FROM `guest` WHERE `firstname` = '$firstname' && `lastname` = '$lastname' && `email` = '$email' && `contactno` = '$contactno'") or die(mysqli_error());
