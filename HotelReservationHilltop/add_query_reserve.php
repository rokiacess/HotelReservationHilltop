<?php
	...
	if(ISSET($_POST['add_guest'])){
		...
		$checkin = $_POST['date'];
		$conn->query("INSERT INTO `guest` (firstname, middlename, lastname, address, email, contactno, day, guest_total) VALUES('$firstname', '$middlename', '$lastname', '$address', '$email','$contactno','$day','$guest_total')") or die(mysqli_error());
		$query = $conn->query("SELECT * FROM `guest` WHERE `firstname` = '$firstname' && `lastname` = '$lastname' && `email` = '$email' && `contactno` = '$contactno' && `day` = '$day' && `guest_total` = '$guest_total'") or die(mysqli_error());
